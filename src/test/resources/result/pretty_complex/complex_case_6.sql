...
SELECT 
    ...
    (SELECT 
         ...
         'price' , 
         h.price))
     FROM inventory_historyh
     WHERE h.product_id = ia.id
         AND h.date >= DATE'2024-01-01'
     ) AS historical_data
FROM inventory_analysisia
    LEFT JOIN supplier_performancesp
    ON ia.id = sp.id
    LEFT JOIN category_metricscm
